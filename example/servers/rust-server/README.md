...
- API version: version not set.0.0
- Build date: 2018-01-27T01:10:47.153Z
...
```
cargo run --example client MakeHat
```
...
* Add `swagger_client = {version = "version not set.0.0", path = "swagger_client"}` under `[dependencies]` in the root `Cargo.toml`.
